...
function runBraidGeometry(payload) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      child.kill("SIGKILL");
      reject(Object.assign(new Error("geometry_timeout"), { statusCode: 504 }));
    }, 20000);
    ...
    child.stdout.on("data", (chunk) => {
      ...
      if (stdout.length > 18 * 1024 * 1024) {
        child.kill("SIGKILL");
        reject(Object.assign(new Error("geometry_output_too_large"), { statusCode: 413 }));
      }
    });
...
function normalizeGeometryPayload(body = {}) {
  ...
  return {
    ...
    ribbonShader: Boolean(body.ribbonShader),
